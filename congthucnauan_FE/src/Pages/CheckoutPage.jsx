...
export default function CheckoutPage() {
    ...
    const paymentMethods = [
        ...
        },
        {
            ...
            icon: 'fas fa-qrcode',
            color: '#6f42c1',
            title: 'SePay - VietQR / Chuyển khoản',
            description: 'Quét mã QR hoặc chuyển khoản ngân hàng - Không phí'
        },
        {
            value: 'bank_transfer',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // If MoMo payment
                ...
                const response = await fetch('/api/v1/momo/create-payment', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`,
            ...
            // If SePay payment
            else if (formData.payment_method === 'sepay') {
                const response = await fetch('/api/v1/sepay/create-payment', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`,
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                {formData.payment_method === 'sepay' && (
                                    <div className="alert alert-info">
                                        ...
                                        <p className="mb-2">Bạn sẽ được chuyển đến trang thanh toán SePay.</p>
                                        <ul className="mb-0">
                                            <li>Quét mã VietQR hoặc chuyển khoản ngân hàng</li>
                                            <li>Tiền chuyển thẳng vào tài khoản — không phí trung gian</li>
                                            <li>Tự động xác nhận thanh toán trong vài giây</li>
                                        </ul>
                                    </div>
                                )}
