...
export default function AdminContactsPage() {
    ...
    const fetchContacts = async () => {
        try {
            const res = await fetch(`/api/v1/admin/contacts?status=${filterStatus}`, {
    ...
    const showContactDetail = async (contact) => {
        const res = await fetch(`/api/v1/admin/contacts/${contact.id}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await res.json();
    ...
    const handleMarkRead = async (id) => {
        await fetch(`/api/v1/admin/contacts/${id}/status`, {
            method: 'PUT',
    ...
    const handleDelete = async (id, name) => {
        ...
        await fetch(`/api/v1/admin/contacts/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}` }
        });
